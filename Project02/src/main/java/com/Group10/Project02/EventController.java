...
class EventController {
    ...
    @GetMapping("/events")
    List<Event> all() {
        return repository.findAllSortDate();
    }

    // Create queries for searching events
    // Single
    @GetMapping(value = "/events", params = "after")
    List<Event> getEventsAfter(@RequestParam("after") String date) {
        return repository.findAfter(date);
    }
