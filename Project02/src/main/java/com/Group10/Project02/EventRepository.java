...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("select e from Event e where e.hostId = ?1")
    ...
    @Query("select e from Event e where e.location = ?1 and e.hostId = ?2")
    ...
    @Query("select e from Event e where e.hostId = ?1 and e.date = ?2")
    List<Event> findByHostIdDate(Long id, String date);

    // Triples

    @Query("select e from Event e where e.location = ?1 and e.hostId = ?2 and e.date = ?3")
